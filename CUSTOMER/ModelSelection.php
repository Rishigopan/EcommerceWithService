                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-md-9 col-lg-9">
                                    ...
                                    <div id="main_list">
                                        ...
                                        $ServiceSelected = $_SESSION['ServiceSelected'];
                                        $series_query = mysqli_query($con, "SELECT B.brand_name,SE.series_name,SE.se_id FROM products P INNER JOIN brands B ON P.brand = B.br_id INNER JOIN series SE ON P.series = SE.se_id WHERE P.brand = '$brand' AND P.isServiceItem = 1 GROUP BY SE.se_id ORDER BY SE.series_name");
                                        if (mysqli_num_rows($series_query) > 0) {
                                            foreach ($series_query as $series) {
                                                $phone_series_id = $series['se_id'];
                                        ?>
                                                <ul class="brand_list list-unstyled my-3">
                                                    <p class="ps-1"><?php echo $series['brand_name']; ?> > <?php echo $series['series_name']; ?></p>
                                                    <?php
                                                    $model_query = mysqli_query($con, "SELECT P.pr_id,P.name,P.img FROM products P INNER JOIN series SE ON P.series = SE.se_id WHERE P.series = '$phone_series_id' AND P.isServiceItem = 1 ORDER BY P.name ASC");
