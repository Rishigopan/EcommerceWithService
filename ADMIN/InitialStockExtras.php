...
//Display initial stock cart items
if (isset($_POST["action"])) {
    ...
    ?>
        <tr>
            ...
            <td><input type="number" min="0" id="<?php echo $Cart['tempPrid']; ?>" class="form-control numberInput change_imei text-center px-1 m-0" value="<?php echo $Cart['imei'];?>"></td>
            <!-- <td><input type="text" id="<?php echo $Cart['tempPrid']; ?>" class="form-control numberInput change_color text-center px-1 m-0" value="<?php echo $Cart['color'];?>"></td> -->
            <td>
                <select id="<?php echo $Cart['tempPrid']; ?>" class="form-select numberInput change_color px-1 m-0">
                    <?php  
                        echo '<option selected value="'.$Cart["color"].'">'.$Cart["TEMPCOLOR"].'</option>';
                        $ListColors = mysqli_query($con, "SELECT colorId,colorName FROM color");
                        foreach($ListColors as $ListColorsResult){
                            echo '<option value="'.$ListColorsResult["colorId"].'">'.$ListColorsResult["colorName"].'</option>';
                        }
                    ?>
                </select> 
            </td>
    ...
    <?php
            ...
            ?>
                ...
                <tr>
                    <td class="text-center" colspan="8"> <strong>Please add some products</strong> </td>
                </tr>
...
//Select Products in initial stock
if (isset($_POST["selectProduct"])) {

    $fetch_selectProduct = mysqli_query($con, "SELECT parent_item_id,name FROM products WHERE pr_id = parent_item_id");

    echo '<option value="" hidden>Select a Product</option>';
    if (mysqli_num_rows($fetch_selectProduct) > 0) {
        foreach ($fetch_selectProduct as $selectProduct) {
        ?>
            <option value="<?php echo $selectProduct['parent_item_id']; ?>"> <?php echo  $selectProduct['name']; ?> </option>
