                        ...
                        <div class="card card-body shadow-sm mb-5" id="main_card">
                            <div class="row">
                                ...
                                <div class="col-12 col-md-9 col-lg-9">
                                    <div>
                                        <h5 class="border-bottom pb-2">Choose Type</h5>
                                    </div>
                                    <div class=" row  mx-md-0 pt-3" id="brand_list">
                                        ...
                                        $ProductTypeQuery = mysqli_query($con, "SELECT PT.productTypeId,PT.productTypeName,PT.productTypeImage FROM products P INNER JOIN types T ON P.type = T.ty_id INNER JOIN brands B ON P.forBrand = B.br_id INNER JOIN series S ON P.forSeries = S.se_id INNER JOIN products Pdisplay ON P.forProduct = Pdisplay.pr_id INNER JOIN producttype PT ON P.productType = PT.productTypeId LEFT JOIN color C ON P.color = C.colorId WHERE T.ty_id = '$SelectionId' AND P.forBrand = '$Brand' AND Pdisplay.pr_id = $Model GROUP BY PT.productTypeId ORDER BY PT.productTypeName ASC");
                                        if (mysqli_num_rows($ProductTypeQuery) > 0) {
                                            foreach ($ProductTypeQuery as $ProductTypeQueryResults) {
                                                $ProductType = $ProductTypeQueryResults['productTypeId'];
                                               
                                        ?>
                                                <div class="col-6 col-md-4 col-xl-3">
                                                    <a href="<?= ($SelectionId == 25) ? 'ServiceSession.php?ProductType='.$ProductType.'&HasColor=YES' : 'ServiceSession.php?ProductType='.$ProductType.''  ?>">
                                                        <div class="card card-body brands mb-3 ">
                                                            <div class="d-block text-center ">
                                                                <img src="../assets/img/TYPE/<?php echo $ProductTypeQueryResults['productTypeImage']; ?>" alt="ProductType">
                                                                <h6 class=" "><?php echo $ProductTypeQueryResults['productTypeName']; ?></h6>
                                                            </div>
                                                        </div>
                                                    </a>
